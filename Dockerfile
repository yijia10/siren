...
RUN echo 'lihua' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/lihua.sh

RUN echo 'cd ' >>/lihua.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/lihua.sh

RUN echo 'cd /noVNC-1.2.0' >>/lihua.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/lihua.sh

RUN chmod 755 /lihua.sh
